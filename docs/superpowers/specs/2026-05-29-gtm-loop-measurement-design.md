...
- **New table `engagement_events`** — mirrors `outcome_events` shape: `id PK, deal_id, source, source_event_id, source_payload_hash, kind, occurred_at, payload_json, created_at, UNIQUE(deal_id, source, source_event_id), CHECK(kind IN (...)), CHECK(source IN ('sales_observed','sales_window_evaluator'))`. Append-only, idempotent **per deal** by `(deal_id, source, source_event_id)` (the same Sales `eventId` may legitimately recur across different deals); changed payload on same `(deal_id, source, source_event_id)` → `idempotency_violation`.
- **New table `commercial_signals`** — non-authoritative observations for the reconciliation queue: `id PK, deal_id, source, source_event_id, source_payload_hash, kind, occurred_at, amount_usd, crm_ref, created_at, UNIQUE(deal_id, source, source_event_id), CHECK(source IN ('sales_reported'))`. **Never writes `commercial_states`.** Surfaced in the console reconciliation queue for an operator/HubSpot to confirm.
